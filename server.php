...
$db = mysqli_connect('localhost', 'root', 'pgadmin', 'knittingdb');
...
if (isset($_POST['reg_user'])) {
  ...
  $user_check_query = "SELECT * FROM users WHERE USER_ID='$username' OR USER_NAME='$username' OR email='$email' LIMIT 1";
  $result = mysqli_query($db, $user_check_query);
  $user = mysqli_fetch_assoc($result);
  
  if ($user) { // if user exists
    if ($user['USER_ID'] === $username || $user['USER_NAME'] === $username) {
