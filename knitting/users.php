...
// Handle Production action
if (isset($_POST['production_user_id'])) {
    $userId = intval($_POST['production_user_id']);
    
    // Get user data
    $getUserQuery = "SELECT id, username, email, password, created FROM users WHERE id = $userId";
    $userResult = mysqli_query($db, $getUserQuery);
    
    if ($userResult && mysqli_num_rows($userResult) > 0) {
        $userData = mysqli_fetch_assoc($userResult);
        
        // Insert into users_update table
        $insertQuery = "INSERT INTO users_update (id, username, email, password, created, production_time) 
                        VALUES ('{$userData['id']}', '{$userData['username']}', '{$userData['email']}', 
                                '{$userData['password']}', '{$userData['created']}', NOW())";
        ...
        if (mysqli_query($db, $insertQuery)) {
            ...
            if (mysqli_query($db, $deleteQuery)) {
                echo "<script>
                    alert('User moved to production successfully!');
            ...
            echo "<script>alert('Error moving to production: " . mysqli_error($db) . "');</script>";
        }
    }
}

// Handle Cancel action - just close the modal
if (isset($_POST['cancel_action'])) {
    // Just redirect back to users page
    header('Location: users.php');
    exit();
}

// Fetch all users from database
$query = "SELECT id, username, email, password, created FROM users ORDER BY id DESC";
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .btn-production {
            background: linear-gradient(95deg, #2e7d32, #43a047);
            color: white;
            padding: 12px 35px;
            border: none;
            border-radius: 16px;
...
<body>
        ...
        <div class="glass-panel">
            <div class="stats-bar">
                ...
                <div class="stats-count">
                    Total Users: <span><?php echo mysqli_num_rows($result); ?></span>
                </div>
            </div>
                            ...
                            <?php while ($row = mysqli_fetch_assoc($result)): ?>
                                <tr>
                                    ...
                                    <td class="user-created">
                                        <?php echo htmlspecialchars(date('d-m-Y H:i', strtotime($row['created']))); ?>
                                    </td>
                                    <td>
        ...
        <div class="qr-modal-content">
            ...
            <div class="qr-user-info" id="qrUserInfo">
                ...
                <div class="info-item">
                    ...
                    <span class="info-value" id="qrCreated">-</span>
                </div>
            </div>
            
            <!-- Production and Cancel Buttons -->
            <div class="qr-action-buttons">
                <button class="btn-production" onclick="moveToProduction()">
                    Production
                </button>
    ...
    <!-- Hidden form for production action -->
    <form id="productionForm" method="post" style="display:none;">
        <input type="hidden" name="production_user_id" id="productionUserId" value="">
    </form>
    ...
    <script>
        ...
        function scanQR(userData) {
            ...
            document.getElementById('qrCreated').textContent = formatDate(userData.created);

            // Store user ID for production action
            document.getElementById('productionUserId').value = userData.id;
        ...
        function moveToProduction() {
            if (confirm('Are you sure you want to move this user to production? This will delete the user from the current list.')) {
